...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ content, onNext, onBack, isFirst, isLast }) => {
  ...
  const handleTimeUpdate = () => {
    ...
    //   const diff = audioRef.current.currentTime - audioRef.current.currentTime;
    //   if (Math.abs(diff) > 0.2) {
      ...
      <div className="absolute inset-x-0 bottom-0 p-8 z-30 bg-gradient-to-t from-black via-black/60 to-transparent pt-40" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={onBack}
            disabled={isFirst}
            className={`w-14 h-14 flex items-center justify-center rounded-2xl glass-panel text-white transition-all active:scale-90 ${isFirst ? 'opacity-10 cursor-not-allowed' : 'hover:bg-white/10 active:bg-white/20'
              }`}
          >
          ...
          <button
            onClick={onNext}
            disabled={isLast}
            className={`w-14 h-14 flex items-center justify-center rounded-2xl glass-panel text-white transition-all active:scale-90 ${isLast ? 'opacity-10 cursor-not-allowed' : 'hover:bg-white/10 active:bg-white/20'
              }`}
          >
